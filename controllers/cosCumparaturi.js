...
const stergeProdus = async (req, res) => {
  ...
  req.session.cos = req.session.cos || [];
  req.session.cos = req.session.cos.filter(
    (parfum) => parfum.produs._id !== produsStersButton,
  );
  req.session.cos = req.session.cos || [];
  req.session.save();
  return res.redirect("/cos");
...
const paginaComanda = async (req, res, next) => {
  ...
  try {
    ...
    for (const produs of produss) {
      const variatie = cos.find(
        (v) => v.produs._id.toString() === produs._id.toString(),
      );
      if (variatie) {
    ...
    if (cos.length === 0) {
      const variante = await modelProdus.find().distinct("categorie").sort();
      const mesajEroare =
        "Cosul de cumparaturi este gol, va rugam sa selectati minim un produs";
      return res.render("cos", { mesajEroare, variante });
    } else {
      return res.render("comanda", {
        cos: produss,
        detalii: detalii,
        totalPret: totalPret,
      });
